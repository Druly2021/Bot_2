...
class Purchase:
    def __int__(self, item_name, price):
        self.item_name = item_name
        self.price = price
        self.purchase_data = datetime.now().strftime("%d-%m-%Y")

    def __str__(self):
        return f"{self.item_name} - {self.price} руб. ({self.purchase_data})"


class Client:
    def __init__(self, user_id, first_name, last_name, bird_data, card_id):
...
def add_client(user_id, first_name, last_name, bird_data, card_id):
    examination_client = next((client for client in clients if str(client.card_id) == str(card_id)), None)
    if examination_client:
        bot.send_message(user_id, "Клиент с указанным идентификатором уже существует")
    else:
        new_client = Client(user_id, first_name, last_name, bird_data, card_id)
...
def send_welcome(message: Message) -> None:
    ...
    - Чтобы добавить клиента отправьте в одном сообщении /add_client Имя, Фамилия, Дата рождения (ДД.ММ.ГГГГ),
      Номер документа удостоверяющего личность
    - Для просмотра базы клиентов введите /show_base
    - Для просмотра подробной информации о клиенте введите /client_info и номер клиента
...
def additive_add_client(message: Message) -> None:
    user_id: int = message.chat.id
    ...
    if not text:
        ...
        client_data = text.strip(" ,")  # Для разделения текста
...
def client_info(message: Message) -> None:
    ...
    try:
        ...
        full_info = (
            ...
            f"Номер удостоверения личности:{client_info.card_id}\n"
        )
...
def additive_add_purchase(message: Message) -> None:
    """Добавляет товар"""
    user_id: int = message.chat.id
